    ...
    if (!selector) {
      ...
      .on('click', function(d, i) {
        if (thatWin.state!=UIState.Connecting) {
          ...
          if (linkStart.direction==VVVV.Core.Pin.Direction.Output)  
            var targetDir = 'input';
          else
          ...
          chart.selectAll('g.vvvv-'+targetDir+'-pin')
            ...
            .append('svg:rect')
              ...
              .attr('height', 3)
              .attr('fill', 'rgba(0,0,0,1)')
              .attr('y', function(d) { return d.direction==VVVV.Core.Pin.Direction.Input ? -3 : 3})
        }
        else {
          
          if (linkStart.direction==VVVV.Core.Pin.Direction.Input) {
            var srcPin = d;
...
VVVV.Editors.BrowserEditor.Inspector = function(VVVVRoot) {
  ...
  function addPin(p) {
    ...
    if (VVVV.PinTypes[p.typeName].openInputBox && p.direction!=VVVV.Core.Pin.Direction.Output && !p.isConnected() && p.getValue(0)!=undefined) {
  ...
  function showAllSlices(p) {
    ...
    if (p.direction!=VVVV.Core.Pin.Direction.Input)
